      ...
      <Card as="section" className={styles.wrapper}>
          ...
          >
            {range(numOfGroups).map((groupIndex) => (
              <div key={groupIndex} className={styles.group}>
                {range(numOfItemsPerGroup * groupIndex + 1, numOfItemsPerGroup * (groupIndex + 1) + 1).map((index) => {
                  return (
                    <motion.div
        ...
        {includeRemainderArea && (
          <div className={styles.remainderArea}>
            ...
            {range(numOfItems - remainder + 1, numOfItems + 1).reverse().map((index) => {
              return (
                <motion.div key={`${instanceId}-${index}`} className={styles.item} layoutId={`${instanceId}-${index}`} />
              );
            })}
          </div>
        )}
